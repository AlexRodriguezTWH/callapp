...
<head>    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css">
  ...
  <div class="row">
 <!--  Tickets pendientes e historial -->
     <div class="col-sm-12 col-md-6 col-lg-6">
    <table class="table table-striped table-sm">
      <tbody id="vista_pediente_historial">
          ...
          <tr>
            <td>{{ Form::label('inpt_codigos', 'Código de identificación') }}</td>
            <td colspan="2">
               <div class="row">
                  <div class="col-md-6">
                      <input type="text" id="codigoFiltro_txt" class="form-control" placeholder="Busca código" value="<?php echo isset($ticket->CodigoIdentificacion) ? $ticket->CodigoIdentificacion : ''; ?>">
                  </div>
               <div class="col-md-6">
          ...
          <tr>
            <td> {{ Form::label('PuntoVenta', 'Punto de venta') }}</td>
            <td> {{ Form::select('PuntoVenta',  $puntos,  $ticket->PuntoVenta ?? null, ['class' => 'form-control form-control-sm', 'id'=>'inpt_puntos']) }}  </td>
          </tr>
          <tr>
            <td> {{ Form::label('Ruta', 'Ruta') }}</td>
            <td> {{ Form::select('Ruta', $rutas,  $ticket->Ruta ?? null, ['class' => 'form-control form-control-sm', 'id'=>'inpt_rutas']) }} </td>
          </tr>
          <tr>
    ...
    <table class="table table-borderless">
      ...
      <tr>
        ...
        <td>
          ...
          {{ Form::select('IdMotivoLlamada',  isset($motivo)? [$motivo->IdMotivoLlamada => $motivo->Descripcion] : [''=>'Selecciona un motivo de llamada'],  $ticket->IdMotivoLlamada, ['class' => 'form-control form-control-sm', 'id'=>'inpt_motivos_edit']) }}
      ...
      <tr>
        <td>
          ...
          {{ Form::select('IdFallaxCliente',  $fallasCliente,  !isset($ticket->IdFallaxCliente)? null: $ticket->IdFallaxCliente, ['class' => 'form-control form-control-sm']) }}
        </td>
        <td>
          {{ Form::label('IdFallaxTecnico', 'Falla Tecnico') }}
          {{ Form::select('IdFallaxTecnico', $fallasTecnico ,  !isset($ticket->IdFallaxTecnico)? 37: $ticket->IdFallaxTecnico, ['class' => 'form-control form-control-sm']) }}
        </td>
      </tr>
...
<tr  id="trCerradaReal" style="{{ ($muestraCerradoTr != 'S') ? 'display:none;' : '' }}">
    ...
    <td>
      {{ Form::label('Hora_Inicial_Hora', 'Hora cierre real') }}
    ...
    {{ Form::select('Hora_Final_Minutos', $minutos, $minuto, [
        ...
        <tr>
          <td colspan="2">
            ...
            <input type="image" src="{{ asset('img/btnAnular2.png') }}" border="0" data-dismiss="modal" class="float-right mt-4 m-2" />
...
//document.addEventListener('DOMContentLoaded', function () {
    ...
    if(fechaFinal && horaFinalHora && horaFinalMinuto){
          ...
          fechaFinal.addEventListener('change', function () {
              ...
              if(fechaInicial.value.trim() == ""  && tipoServicio == 5){
                  ...
                  const fechaCreacion = new Date(obtieneCreacion.replace(" ", "T"));
                  const fechaActual = fechaCreacion.toLocaleDateString("en-CA");
  ...
  document.getElementById('codigoFiltro_txt').addEventListener('input', function () {
      console.log("ENTRO EN ODGIO FILTRO");
